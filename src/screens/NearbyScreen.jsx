...
import { MOCK_USERS, INTENT_CONFIG } from "../data/mockUsers";
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:"#0C0C14", color:"#F0EEF8", minHeight:"100vh", maxWidth:390, margin:"0 auto", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#1A0A2E 0%,#0D1A3A 50%,#0A1A1A 100%)", padding:"44px 16px 14px", display:"flex", alignItems:"center", gap:12, position:"relative", overflow:"hidden", flexShrink:0 }}>
        ...
        <div style={{ background:"rgba(52,211,153,0.15)", border:"1px solid rgba(52,211,153,0.4)", color:"#34D399", fontSize:11, fontWeight:500, padding:"3px 10px", borderRadius:20 }}>● 24 active</div>
      </div>
      ...
      <div style={{ margin:"0 14px", borderRadius:20, overflow:"hidden", position:"relative", flexShrink:0, height:200, background:"#0D1A1A", border:"1px solid rgba(255,255,255,0.07)" }}>
        <div style={{ width:"100%", height:"100%", position:"relative", background:"linear-gradient(135deg,#0A1A14 0%,#0D1F1A 40%,#091510 100%)" }}>
          ...
          {MOCK_USERS.map(u => {
            const s = INTENT_CONFIG[u.intent];
            const isFiltered = activeFilter !== "All" && !activeFilter.includes(u.intent);
            return (
              <div key={u.id} style={{ position:"absolute", display:"flex", flexDirection:"column", alignItems:"center", cursor:"pointer", transition:"all 0.2s", top:u.pin.top, left:u.pin.left, opacity: isFiltered ? 0.2 : 1 }}>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"12px 14px 16px" }}>
        ...
        {MOCK_USERS.filter(u => activeFilter === "All" || activeFilter.includes(u.intent)).map(u => {
          const s = INTENT_CONFIG[u.intent];
          return (
            ...
            >
              ...
              <div style={{ position:"relative", width:44, height:44, flexShrink:0 }}>
                <div style={{ width:44, height:44, borderRadius:"50%", background:u.avatarBg, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Syne',sans-serif", fontWeight:700, fontSize:14, color:u.avatarColor }}>{u.initials}</div>
                <div style={{ position:"absolute", bottom:1, right:1, width:9, height:9, borderRadius:"50%", background: u.online ? "#34D399" : "#FBBF24", border:"2px solid #0C0C14" }} />
              </div>
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontWeight:500, fontSize:14, color:"#EEEAF8" }}>{u.name}</div>
                <div style={{ fontSize:11, color:"rgba(180,175,210,0.5)", marginTop:2 }}>📍 {u.distance} · {u.walk} · {u.time}</div>
              </div>
